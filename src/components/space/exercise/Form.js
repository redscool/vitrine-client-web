...
export default function Form() {
  ...
  const [isMCQDraggable, setIsMCQDraggable] = useState(false);
  ...
  const mcqDragHandler = (source, dest) => {
    ...
    // tties[id].splice(dest.index, 0, ele);
  ...
  const onDragStart = (result) => {
    if (result.source.droppableId !== "dnd-form-1") return;
    setIsMCQDraggable(false);
  };
