...
import styles from "../../../styles_v2/components_v2/space/calendar/AddEventPopup.module.css";
  ...
  const handleClick = () => {
    ...
    serviceObject.request(
      ...
      (response) => {
        ...
        if(status == 204) { // integrate google again 
          /*
          - handle google auth: (done)
          - generate google installation url (done)
          - open google consent page (done)
          - create a callback page for google redirect (done)
          - hit backend api with auth code (done)
          */
        }else{
        ...
        switch message:
            case ("SUCCESS"):
              - handle event successfully created:
              - show success
              - refetch events
            case ("Integrate google again"): (204)
              - handle google auth:
              - generate google installation url
              - open google consent page
              - create a callback page for google redirect
              - hit backend api with auth code
              - check google auth success
              - if success => hit create event api again, otherwise => show error
        */
      }
      ,
      console.log
    ...
    <div className={`${styles.mainContainer}`} onClick={() => setView(false)}>
      ...
      >
        ...
        <div className={styles.button} onClick={() => {
          const stringifiedParams = queryString.stringify(config.GOOGLE_INTEGRATION);
